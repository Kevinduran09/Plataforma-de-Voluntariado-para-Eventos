...
export const ItemPillComponent: React.FC<{
    ...
    return (
        ...
        >
            <div className='flex justify-items-stretch gap-3 items-center'>
                <img
                ...
                />
                <span>{evento.evento.nombre}</span>
                •
                <span className="text-sm text-gray-500">
                    {new Date(evento.fecha_inscripcion).toLocaleDateString()}
                </span>
                •
                <span className="text-sm text-gray-500">{evento.evento.lugar}</span>
            </div>
        </li>
    );
};