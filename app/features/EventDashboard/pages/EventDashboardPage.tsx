...
import { ItemListComponent } from '../ui/ItemListComponent';
...
export async function clientLoader() {
    const data: Evento[] = await GetEventDashboard()

    if (!data) {
        throw new Error('Error al cargar los datos')
    }

    return { data: data.slice(0, 20),age:21 }
}


export default function EventDashboardPage() {
    const { data,age } = useLoaderData()

    return (
        <div className="flex flex-row justify-center items-center">
            <h2>{age}</h2>
            <div className='flex flex-col  items-stretch gap-4 p-3 justify-stretch w-1/2'>
                {
                    data.map((item: Evento) => {
                        return <ItemListComponent key={item.id} item={item} />
                    })
                }
            </div>
            ...
            <div data-dial-init className="fixed end-6 bottom-6 group">
                <div id="speed-dial-menu-default" className="flex flex-col items-center hidden mb-4 space-y-2">
                    ...
                    <div id="tooltip-share" role="tooltip" className="absolute z-10 invisible inline-block w-auto px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-xs opacity-0 tooltip dark:bg-gray-700">
                        Share
                    ...
                    <div id="tooltip-print" role="tooltip" className="absolute z-10 invisible inline-block w-auto px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-xs opacity-0 tooltip dark:bg-gray-700">
                        Print
                        <div className="tooltip-arrow" data-popper-arrow></div>
                    </div>
                    ...
                    <div id="tooltip-download" role="tooltip" className="absolute z-10 invisible inline-block w-auto px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-xs opacity-0 tooltip dark:bg-gray-700">
                        Download
                        <div className="tooltip-arrow" data-popper-arrow></div>
                    </div>
                    ...
                    <div id="tooltip-copy" role="tooltip" className="absolute z-10 invisible inline-block w-auto px-3 py-2 text-sm font-medium text-white transition-opacity duration-300 bg-gray-900 rounded-lg shadow-xs opacity-0 tooltip dark:bg-gray-700">
                        Copy
                        <div className="tooltip-arrow" data-popper-arrow></div>
                    </div>
                </div>
